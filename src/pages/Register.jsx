...
function Register() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // ✅ เน้นว่า “ต้องไปยืนยันในอีเมลก่อน”
      const msgFromServer = res?.message;
      alert(
        msgFromServer ||
          'สมัครเกือบสำเร็จแล้ว! ระบบได้ส่งอีเมลให้คุณยืนยัน กรุณาคลิกปุ่มยืนยันในอีเมลก่อนเข้าสู่ระบบ'
      );
      ...
      // พาไปหน้า Login ให้ผู้ใช้ลองล็อกอินหลังยืนยันอีเมลแล้ว
      navigate('/login');
    } catch (err) {
  ...
  // Shared Input Styles
  const inputClass =
    'px-3 py-2.5 rounded-lg border border-gray-300 text-sm outline-none transition focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400/30 w-full';
  const labelClass =
    'flex flex-col gap-1.5 text-sm font-semibold text-gray-700';
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4 font-sans text-gray-900">
      <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 w-full max-w-[420px]">
        ...
        <p className="text-sm text-gray-500 mb-6">
          Sign up to use the PM Dashboard system.
        </p>
