...
// -------------------------------โค้ดด้านบนนี้เป็นโค้ดที่ใช้งานได้ปกติยกเว้นฟังค์ชั้นการดาวโหลด --------------------------------------------
  ...
  const downloadCSV = async () => {
    ...
    try {
      const snap = await get(
        child(ref(database), `predictions/${date}`)
      );
    ...
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-4 py-3 text-right">
        <button
          onClick={downloadCSV}
          disabled={loading}
          className="px-3 py-1.5 bg-emerald-500 text-white rounded flex gap-2 items-center ml-auto"
        >
          {loading
...
export default function DataDownload() {
  ...
  useEffect(() => {
    const loadDates = async () => {
      try {
        const snap = await get(child(ref(database), "predictions"));

        if (snap.exists()) {
          const keys = Object.keys(snap.val());
          setDates(keys.sort().reverse());
        }
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 md:ml-[240px] pt-14 p-6">
